...
PRICING_CONFIG = {
    ...
    # You said you won't be hands-on, so labour is part of the pricing model.
    "helper_fee_per_person": 220,
    "fragile_handling_fee": 120,
    "platform_margin": 0.12,
    "global_minimum_job_charge": 350,
}

def round_to_nearest_10(amount: float) -> int:
    return int(ceil(amount / 10.0) * 10)
...
def determine_vehicle(data: dict) -> str:
    ...
    if load_type == "home_office_load":
        return "outsourced_truck" if load_size in ["large", "extra_large"] else "np200_trailer_hired"
    ...
    if service_type in ["document_courier", "same_day_delivery"]:
        if load_type in ["documents", "small_parcel", "retail_bags"] and load_size in ["small", "medium"] and item_count <= 3:
            return "bike"
        return "np200"

    if service_type == "store_pickup":
        if load_type in ["documents", "small_parcel", "retail_bags"] and load_size == "small" and item_count <= 4:
            return "bike"
    ...
    if service_type == "purchase_collection":
        if load_type in ["documents", "small_parcel", "retail_bags"] and load_size == "small" and item_count <= 3:
            return "bike"
...
def determine_helpers(data: dict, vehicle_key: str, effort_level: str) -> int:
    ...
    if effort_level == "low":
        return 1 if load_type in ["furniture", "appliance"] else 0
    ...
    if effort_level == "heavy":
        return 3 if load_size == "extra_large" else 2

    return 1


def estimate_internal_cost(vehicle_key: str, operational_distance_km: float, helpers_required: int, trailer_hire_buffer: float, outsourcing_buffer: float) -> dict:
    vehicle_cost_per_km = {
        "bike": 2.50,
        "np200": 4.50,
    ...
    total_cost = distance_cost + driver_cost + helper_cost + trailer_hire_buffer + outsourcing_buffer
...
def calculate_quote(data: dict, route: dict) -> dict:
    ...
    dead_mileage_weight = 0.50
    chargeable_distance_km = customer_distance_km + (dead_mileage_km * dead_mileage_weight)

    base_fee = vehicle["base_fee"]
    distance_charge = chargeable_distance_km * vehicle["rate_per_km"]
    ...
    fragile_fee = PRICING_CONFIG["fragile_handling_fee"] if fragile_value in [True, "yes", "mixed"] else 0
    ...
    final_total = max(round_to_nearest_10(subtotal + platform_margin), vehicle["minimum_charge"], PRICING_CONFIG["global_minimum_job_charge"])
    ...
    estimated_margin_percent = (estimated_profit / final_total * 100) if final_total > 0 else 0

    return {
        "pickup": data["pickup"],
        "dropoff": data["dropoff"],
        "service_type": service_label,
        "customer_service_label": customer_service_level(data, vehicle_key, effort_level),
        "load_type": load_label,
        "distance_km": customer_distance_km,
        "operational_distance_km": operational_distance_km,
        "dead_mileage_km": dead_mileage_km,
        "chargeable_distance_km": round(chargeable_distance_km, 1),
            "billing_distance_km": round(chargeable_distance_km, 1),
        "billing_distance_km": round(chargeable_distance_km, 1),
        ...
        "breakdown": {
            ...
            "operational_distance_km": operational_distance_km,
            "dead_mileage_km": dead_mileage_km,
            "dead_mileage_weight": dead_mileage_weight,
            "chargeable_distance_km": round(chargeable_distance_km, 1),
        "billing_distance_km": round(chargeable_distance_km, 1),
        ...
        "disclaimer": (
            "This is a provisional estimate generated from customer specifications. Final pricing may change "
            "after review if item size, access, timing, waiting time or availability differs from the details provided."
        ),
        "whatsapp_url": f"https://wa.me/{WHATSAPP_NUMBER}?text={quote_plus('Hi Moveango, I would like to book this quote.')}",
    }
